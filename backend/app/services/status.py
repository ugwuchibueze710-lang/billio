...
def urgency_for(due_date: date, is_paid: bool, paid_at_local: date | None, today: date) -> UrgencyLabel:
    ...
    if status == STATUS_PAID:
        # Deliberately not using strftime's "day without a leading zero"
        # flag here -- it's spelled "%-d" on Linux/macOS but "%#d" on
        # Windows (both are non-standard platform extensions, not part of
        # the C standard strftime), so any single spelling crashes on one
        # platform or the other. Building the day number directly with
        # str(int(...)) works identically everywhere.
        paid_label = f"Paid {paid_at_local.strftime('%b')} {paid_at_local.day}" if paid_at_local else "Paid"
