      ...
      <div>
        ...
        <BillForm
          ...
          initialValues={{
            ...
            annual_month: bill.month || "",
            annual_day: bill.day_of_month || "",
          ...
          onSubmit={async (values) => {
            ...
            // recurrence itself can't be changed here (see recurrenceLocked
            // above), so which reschedule field to send is decided by the
            // bill's actual stored recurrence, not the form -- monthly and
            // yearly bills never need a year, everything else keeps using
            // an explicit due date.
            if (bill.recurrence === "monthly") {
              payload.day_of_month = values.day_of_month || undefined;
            } else if (bill.recurrence === "yearly") {
              payload.annual_month = values.annual_month || undefined;
              payload.annual_day = values.annual_day || undefined;
            } else {
              payload.due_date = values.due_date || undefined;
            }
            await api.bills.update(billId, payload);
